...
var pageManager = function(router, states) {
    ...
    this.controllerContext = require.context("../../..", true, /^((?![\/|\\]node_modules[\/|\\]).)*([\/|\\]controllers[\/|\\])(\w|\\|\/)*(\-controller)\.js$/);

    // look for templates folder in the root of the app excluding files in node_modules folder
    this.templateContext = require.context("../../..", true, /^((?![\/|\\]node_modules[\/|\\]).)*([\/|\\]templates[\/|\\])(\w|\\|\/)*(\-template)\.(dust|jade)$/);
}
