...
pageManager.prototype.unloadPreviousState = function(keepIndex) {
    ...
    for (var i = this.currentState.path.length - 1; i > keepIndex - 1; i--) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.loadedControllers, state.name)) {
            var _controllers = this.loadedControllers[state.name];
            
            try{
                _.each(_controllers, function(controller) {
                    controller.unload();
                });
            }catch(err){
                console.log(err);
...
pageManager.prototype.loadState = function(state, queryString) {
    ...
    if (this.stateHistory.length > 1 && this.stateHistory[1] && this.stateHistory[1].query == navigatingTo) {
    ...
    */
    ...
    if(commonStates.length > 0){
        ...
        while(removed < commonStates.length){
            var commonState = commonStates[keepIndex - 1 - removed];
            var same = true;
            
            if(commonState.url.prefix !== "" && this.currentState.cleanUrl.indexOf(commonState.url.prefix) > -1 && state.cleanUrl.indexOf(commonState.url.prefix) > -1){
                _.each(state.params, function(value, key){
                    if(key !== "query"){
                        same = same && Object.prototype.hasOwnProperty.call(this.currentState.params, key) && (this.currentState.params[key] === value);
                    }else{
                        if(Object.keys(value).length !== Object.keys(this.currentState.params.query).length){
                            same = false;
                        }else{
                            _.each(value, function(queryVal, queryKey){
                                same = same && Object.prototype.hasOwnProperty.call(this.currentState.params.query, queryKey) && (this.currentState.params.query[queryKey] === queryVal);
                            }.bind(this));
                        }
                    }
                }.bind(this));
        ...
        keepIndex = commonStates.length - removed;
    }
        ...
        .then(function() {
            ...
            if(!this.clickedBack){
                if(window.scrollTo){
					...
					Backbone.$(window).scrollTop(this.scrollTo);	
				}
